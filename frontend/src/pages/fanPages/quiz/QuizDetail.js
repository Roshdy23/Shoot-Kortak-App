                ...
                <h3 style={{marginLeft:"0px"}} className="row mt-4">{quiz.name}</h3>
                    ...
                    <div style={{padding:"15px",display:"flex",flexDirection:"column"}}>
                        <div data-bs-spy="scroll" data-bs-target="#list-example" data-bs-smooth-scroll="true" className="scrollspy-example" tabindex="0" >
                            ...
                            <Button onClick={()=>{let array = answers; array[currInd]=ch; setAnswers(answers); if(s<1){refresh(s+1)}else{refresh(s-1)}}} className={(answers?.at(currInd)===ch)?("selected-option"):("option")}>{ch}</Button>
                            )}
                            </div>
